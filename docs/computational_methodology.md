...
| 2026-07-14 | D011-R1 development-only campaign resume accepted | D011-C2 established that the corrected largest-fold synthetic workload fits the reference laptop limits | The next scientific question requires development-only Q01b/FQK execution, but locked splits and Gate 6 must remain protected | Recorded D011-R1 as a one-campaign authority using the frozen D011 protocol, preserving D011/D011-C1 stops and D011-C2 admission as separate evidence | Pending execution at freeze time. Development rows may be opened only for this frozen campaign; calibration/final-test rows, hardware/GPU, Gate 5 reinterpretation, and Gate 6 remain unauthorized |
