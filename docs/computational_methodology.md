...
| 2026-07-13 | D011 pre-execution fold-shape audit and conditional authority | D010's valid frozen bundle predicted 256 validation rows, but the real grouped folds contain 6,500 or 9,750 held-out rows and every complete task predicts 39,000 rows | Using the D010 projection as final campaign admission could understate validation projection/inference time even though its original PASS remains valid | Froze a q=8, two-layer, 1,024-training/9,750-validation synthetic bundle, charged 1,220 worst-fold units with 25% margin and no sharing/cache/smaller-shape credit, reused fold transforms and mathematically identical exact states only during actual execution, and retained live memory/disk/artifact/time guards | Pending preflight. Development data remain unopened until every unchanged ceiling passes. A PASS permits one resumable CPU-only P001 campaign; a STOP is terminal. The execution optimization changes no rows, models, seeds, metrics, or selection rules and all paper-relevant stages reserve RFIG-026 through RFIG-031 |
