...
| 2026-07-13 | D008 exploratory implementation freeze candidate | P001 required exact methods and a way to learn from future failures without post-outcome rescue testing | An underspecified projected kernel or informal failure suggestion could silently change bandwidth, controls, retries, compute scope, or paper interpretation after results | Prepared a 30-pair balanced Q01b/FQK manifest, one-RDM projected-kernel formula, endpoint-specific controls and stop rules, laptop-bounded compute admission, future-research register, and RFIG-025 | Candidate only. Every failure/stop must be committed with an evidence-based future improvement, but that improvement cannot change or retry the active pipeline. No implementation or experiment is authorized until separate human decisions |
