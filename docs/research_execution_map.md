...
| 7. Claims and release | Finish manuscript, perform robustness and negative-result analysis, audit reproducibility, prepare archive, and publish only after human approval | Accept or reject each main claim and public release | D029-C clean clone audit STOP: pytest failed on byte-provenance/hash portability while ruff and compileall passed. RFIG-048 records the STOP. Remaining work: D030-C line-ending/byte-provenance correction, rerun clean audit, final claim review, release decision, tag, and archive |
