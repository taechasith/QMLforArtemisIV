...
| 5X. Exploratory protocol | Freeze and synthetically validate a post-Gate-5 exploratory implementation for Q01b projected quantum kernel and FQK feasibility-only quantum kernel only | D008 accepted and synthetic validation complete; D009 ended in technical `STOP` 2026-07-13; research-data execution unauthorized | D009 stopped after the first shared synthetic projection because Windows peak-memory telemetry failed; no heads, controls, or admission result exists. P001-FR001 and RFIG-029 record the bounded failure; any correction/rerun needs a new prospective decision |
