...
| 5. Development and algorithm trigger | Generate and validate unlocked payloads, run only preregistered development analysis, and test whether the residual-QML trigger passes | In progress: D005 and D006 accepted; development-only fitting authorized, with full scale-up contingent on the bounded benchmark | Pre-D003 F0 excluded; RFIG-002 through RFIG-020 retained; 39,000 development rows pass preflight and the accepted 450-task first stage is ready; source-bound reporting, worst-shape scale gating, final-root containment, and mandatory D004 diagnostic reporting are frozen; calibration/final reads remain zero; trigger report pending |
