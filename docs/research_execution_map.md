...
| 5X. Exploratory protocol | Execute the accepted post-Gate-5 Q01b projected quantum kernel and FQK feasibility-only quantum kernel protocol under corrected fold-shape admission | D011-R1 accepted; single development-only campaign authorized | D009 remains an immutable telemetry STOP, D010 attempt 2 remains a closed synthetic PASS under its own frozen shape, D011 remains a terminal pre-launch STOP from the `scripts` namespace import failure, and D011-C1 remains a terminal authority-hash STOP. D011-C2 passed hash/import smoke tests and corrected synthetic compute admission under the unchanged largest-fold workload. D011-R1 authorizes exactly one frozen development-only campaign. Calibration/final-test, hardware/GPU, Gate 5 reinterpretation, and Gate 6 remain locked |
