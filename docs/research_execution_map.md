...
| P018. Geometric QML feasibility | Keep the D051-C/D052-C future mechanism discussion scientific: bind a mathematical contract, numerical audit method, and future controls before any new model is considered | No audit execution decision exists. A future decision must bind the concrete representation, DLA basis, context/stabilizer, rotations, target scale, strata, raw-output paths, and figure registry before one bounded audit can be authorized | D053-C freezes direct SVD in closed Pauli/DLA coefficient space, three-threshold nullity stability, an explicit basis/resource stop, and the default-versus-tightened DOP853 replay. No DLA, trajectory, data row, fit, figure, or Gate 6 evidence has been created |
