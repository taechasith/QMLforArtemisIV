...
| 5X. Exploratory protocol | Freeze and synthetically validate a post-Gate-5 exploratory implementation for Q01b projected quantum kernel and FQK feasibility-only quantum kernel only | D010 accepted for typed telemetry validation and one unchanged synthetic preflight attempt; research-data execution unauthorized | D009 remains an immutable technical `STOP`. D010 changes telemetry and Git-blob provenance only, requires an independent memory check, and preserves every scientific workload and ceiling. A PASS permits preparation of D011 only; a STOP closes the authorized retry and invokes the future-research firewall |
