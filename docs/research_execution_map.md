...
| 7. Claims and release | Finish manuscript, perform robustness and negative-result analysis, audit reproducibility, prepare archive, and publish only after human approval | Accept release package, request revisions, or reject release | D032-C release-candidate manifest READY: the decision packet is hashed, RFIG-051 records the manifest, and release remains unauthorized. Remaining work after human acceptance only: release tag/archive/DOI/citation metadata preparation |
