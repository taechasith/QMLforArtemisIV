...
| 5X. Exploratory protocol | Freeze and synthetically validate a post-Gate-5 exploratory implementation for Q01b projected quantum kernel and FQK feasibility-only quantum kernel only | D010 synthetic compute admission `PASS`; D011 and research-data execution unauthorized | D009 remains an immutable technical `STOP`. D010 attempt 2 preserved every scientific workload and ceiling, passed typed telemetry validation and all five laptop limits, and is closed to rerun. RFIG-030 records the resource margins; a separate D011 decision is required before development-row fitting |
