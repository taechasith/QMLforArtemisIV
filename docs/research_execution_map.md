...
| 5X. Exploratory protocol | Freeze a post-Gate-5 exploratory implementation for Q01b projected quantum kernel and FQK feasibility-only quantum kernel only | D008 candidate prepared 2026-07-13; human acceptance pending; no implementation or execution authorized | The D008 YAML, implementation-freeze document, 30-row paired manifest, future-research register, and RFIG-025 fix methods and the failure-discussion firewall; even acceptance authorizes only implementation/synthetic validation until a separate clean-source execution decision |
