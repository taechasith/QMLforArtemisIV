...
| 5X. Exploratory protocol | Execute the accepted post-Gate-5 Q01b projected quantum kernel and FQK feasibility-only quantum kernel protocol under corrected fold-shape admission | D011-C2 raw-blob hash correction accepted for smoke tests and one unchanged preflight attempt; development campaign still requires later human decision | D009 remains an immutable telemetry STOP, D010 attempt 2 remains a closed synthetic PASS under its own frozen shape, D011 remains a terminal pre-launch STOP from the `scripts` namespace import failure, and D011-C1 remains a terminal authority-hash STOP. D011-C2 corrects only raw dependency hashes, requires smoke tests, and writes separate preflight evidence. RFIG-031 records reached corrected admission; RFIG-029 updates only for governed stops. Calibration/final-test, hardware/GPU, Gate 5 reinterpretation, and Gate 6 remain locked |
