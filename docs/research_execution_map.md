...
| 7. Claims and release | Finish manuscript, perform robustness and negative-result analysis, audit reproducibility, prepare archive, and publish only after human approval | Release package accepted and published under D033-C | D033-C release package ACCEPTED and PUBLISHED at https://github.com/taechasith/QMLforArtemisIV/releases/tag/v0.3.0: RFIG-052 records the accepted release boundary, `CITATION.cff` is updated to `0.3.0`, and the `v0.3.0` source tag and archive are live. Publication changed no scientific evidence. DOI minting, Gate 6, locked data, mission loop, model release, QML invention claims, and quantum-advantage claims remain unauthorized |
