...
| 7. Claims and release | Finish manuscript, perform robustness and negative-result analysis, audit reproducibility, prepare archive, and publish only after human approval | Release package accepted under D033-C | D033-C release package ACCEPTED: RFIG-052 records the accepted release boundary, `CITATION.cff` is updated to `0.3.0`, and the `v0.3.0` source tag is authorized. DOI minting, Gate 6, locked data, mission loop, model release, QML invention claims, and quantum-advantage claims remain unauthorized |
