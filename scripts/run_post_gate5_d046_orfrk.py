"""Run the D046 orthogonalized residual fidelity-RBF campaign."""
...
CONFIG_PATH = ROOT / "configs/post_gate5_d046_orfrk.yaml"
PHASE_CONFIG_PATH = ROOT / "configs/phase1_benchmark.yaml"


def _orthogonalized_target(residual: np.ndarray, shared_prediction: np.ndarray) -> np.ndarray:
    """Remove only the training-only shared first-stage prediction."""

    first = np.asarray(residual, dtype=float)
    shared = np.asarray(shared_prediction, dtype=float)
    if first.shape != shared.shape:
        raise ValueError("D046 residual and shared prediction shapes do not match")
    if not np.all(np.isfinite(first)) or not np.all(np.isfinite(shared)):
        raise ValueError("D046 residual and shared prediction values must be finite")
    target = first - shared
    if not np.all(np.isfinite(target)):
        raise ValueError("D046 orthogonalized target is not finite")
    return target
...
def run() -> dict[str, Any]:
    ...
    if config.get("decision_id") != "D046" or config.get("protocol_id") != "P014":
        raise RuntimeError("D046 config identity is invalid")
    if not bool(config["authority"]["research_data_fitting_authorized"]):
        raise RuntimeError("D046 fitting is not authorized")
    d045_result = json.loads(
        (ROOT / "data/processed/reporting/post_gate5_d045_msfrk/campaign_result.json").read_text(encoding="utf-8")
    )
    if d045_result.get("status") != "complete_valid_negative":
        raise RuntimeError("D046 requires an integrity-valid D045 negative")
    free_disk = shutil.disk_usage(ROOT).free / 1024**3
    floor = float(config["resources"]["free_disk_floor_gib"])
    if free_disk < floor:
        raise RuntimeError(f"D046 free disk preflight is below {floor:.1f} GiB: {free_disk:.3f} GiB")
    ...
    if result_path.is_file():
        ...
        if result.get("source_commit") != source_commit:
            raise RuntimeError("D046 result belongs to another source commit")
        return result
    ...
    if audit["rows"] != 39000 or audit["unique_scenarios"] != 39000:
        raise RuntimeError("D046 development grain does not match the frozen audit")
    fold_ids = sorted({str(row["fold_id"]) for row in fold_manifest_rows(phase_config, manifest)})
    if fold_ids != ["CV01", "CV02", "CV03", "CV04", "CV05"]:
        raise RuntimeError("D046 outer fold manifest is invalid")
    ...
    for outer_fold in fold_ids:
        ...
        for seed in seeds:
            ...
            outer_predictions: dict[int, tuple[np.ndarray, np.ndarray, np.ndarray]] = {}
            for q in q_values:
                ...
                del fidelity_oof, rbf50_oof
                orthogonal_target = _orthogonalized_target(residual, rbf25_oof)
                ...
                if train_input.shape != (raw_train.shape[0], q + 1) or validation_input.shape != (raw_validation.shape[0], q + 1):
                    raise RuntimeError("D046 conditioning feature shape is invalid")
                ...
                channel_id = f"D046-q{q:02d}"
                first_stage = _fit_rbf_pair(aligned_train, residual, context.row_ids, channel_id, outer_fold, seed, 1.0, 256)
                first_predictions = _predict_rbf_pair(first_stage, aligned_validation)
                second_fidelity = _fit_fidelity(aligned_train, orthogonal_target, context.row_ids, channel_id, outer_fold, seed, q, 1, 1.0, True, 1.0, 256)
                fidelity_delta = _predict_fidelity(second_fidelity, aligned_validation)
                second_rbf = _fit_rbf_pair(aligned_train, orthogonal_target, context.row_ids, channel_id, outer_fold, seed, 1.0, 256)
                second_rbf_predictions = _predict_rbf_pair(second_rbf, aligned_validation)
                first_rbf25 = first_predictions[0.25]
                second_rbf50 = second_rbf_predictions[0.50]
                outer_predictions[q] = (first_rbf25, fidelity_delta, second_rbf50)
                orthogonal_audit.extend(inner_rows)
                channel_audit.append({"outer_fold": outer_fold, "seed_index": seed, "q": q, "orthogonalized_target_l2": float(np.linalg.norm(orthogonal_target)), "first_stage_rbf25_l2": float(np.linalg.norm(first_rbf25)), "second_stage_fidelity_l2": float(np.linalg.norm(fidelity_delta)), "second_stage_rbf50_l2": float(np.linalg.norm(second_rbf50)), "validation_outcomes_used": 0, "stage_order": "shared_rbf25_then_e2_second_stage", "channel_source": "inner_oof_rbf25_orthogonalized_target"})

            for q in q_values:
                first_rbf25, fidelity_delta, second_rbf50 = outer_predictions[q]
                candidate = context.target_standardizer.inverse(c06_standardized_validation + 0.10 * (first_rbf25 + fidelity_delta))
                control = context.target_standardizer.inverse(c06_standardized_validation + 0.10 * (first_rbf25 + second_rbf50))
                for model_id, prediction in ((f"ORFRK-{q:02d}-R2", candidate), (f"TWO-RBF-{q:02d}-R2", control)):
    ...
    primary_id = "ORFRK-08-R2"
    control_id = "TWO-RBF-08-R2"
    ...
    if c06_nrmse.size != 20 or primary_nrmse.size != 20 or control_nrmse.size != 20:
        raise RuntimeError("D046 paired seed endpoint is incomplete")
    interval = paired_bootstrap_mean_interval(primary_nrmse - c06_nrmse, confidence_level=0.95, replicates=10000, seed=46046)
    ...
    result = {"schema_version": "0.1.0", "decision_id": "D046", "protocol_id": "P014", "status": "complete_scientifically_superior" if all(checks.values()) else "complete_valid_negative", "source_commit": source_commit, "development_audit": audit, "development_rows_read": len(records), "calibration_rows_read": 0, "final_test_rows_read": 0, "hardware_jobs_submitted": 0, "gate6_runs": 0, "primary_model": primary_id, "primary_summary": primary_summary, "c06_summary": c06_summary, "classical_orthogonalized_summary": control_summary, "decision_checks": checks, "paired_nrmse_interval": {"estimate": interval.estimate, "lower": interval.lower, "upper": interval.upper, "confidence_level": interval.confidence_level}, "inner_audit_rows": len(inner_audit), "orthogonal_audit_rows": len(orthogonal_audit), "channel_audit_rows": len(channel_audit), "q_values": q_values, "stage_order": "shared RBF-0.25 fitted to e; inner OOF RBF-0.25 defines e2=e-r25_OOF; second-stage fidelity versus RBF-0.50", "runtime": {"wall_clock_seconds": time.perf_counter() - started, "cpu_seconds": time.process_time() - cpu_started, "peak_working_set_gib": process_memory_observation().peak_bytes / 1024**3, "free_disk_gib": shutil.disk_usage(ROOT).free / 1024**3, "free_disk_preflight_gib": free_disk}, "claim_boundary": config["reporting"]["claim_boundary"], "result_files": ["inner_fold_audit.csv", "orthogonal_audit.csv", "channel_audit.csv", "fold_seed_metrics.csv", "per_seed_metrics.csv", "summary.csv", "paired_comparison.csv"]}
