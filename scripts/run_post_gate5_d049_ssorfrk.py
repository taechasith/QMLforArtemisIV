"""Run the D049 stage-separated q8 gain-calibration campaign."""
...
CONFIG_PATH = ROOT / "configs/post_gate5_d049_ssorfrk.yaml"
PHASE_CONFIG_PATH = ROOT / "configs/phase1_benchmark.yaml"


def _select_second_stage_gain(residual: np.ndarray, first: np.ndarray, second: np.ndarray, shared_gain: float, grid: tuple[float, ...]) -> tuple[float, list[float]]:
    ...
    if labels.shape != first_values.shape or labels.shape != second_values.shape:
        raise ValueError("D049 gain arrays do not have matching shapes")
    if not np.all(np.isfinite(labels)) or not np.all(np.isfinite(first_values)) or not np.all(np.isfinite(second_values)):
        raise ValueError("D049 gain arrays are not finite")
    losses = [float(np.mean((labels - float(shared_gain) * first_values - value * second_values) ** 2)) for value in grid]
    if not np.all(np.isfinite(losses)):
        raise ValueError("D049 gain losses are not finite")
...
def run() -> dict[str, Any]:
    ...
    if config.get("decision_id") != "D049" or config.get("protocol_id") != "P017":
        raise RuntimeError("D049 config identity is invalid")
    if not bool(config["authority"]["research_data_fitting_authorized"]):
        raise RuntimeError("D049 fitting is not authorized")
    d048_result = json.loads((ROOT / "data/processed/reporting/post_gate5_d048_nsorfrk/campaign_result.json").read_text(encoding="utf-8"))
    if d048_result.get("status") != "complete_valid_negative":
        raise RuntimeError("D049 requires an integrity-valid D048 negative")
    free_disk = shutil.disk_usage(ROOT).free / 1024**3
    floor = float(config["resources"]["free_disk_floor_gib"])
    if free_disk < floor:
        raise RuntimeError(f"D049 free disk preflight is below {floor:.1f} GiB: {free_disk:.3f} GiB")
    ...
    if result_path.is_file():
        ...
        if result.get("source_commit") != source_commit:
            raise RuntimeError("D049 result belongs to another source commit")
        return result
    ...
    if audit["rows"] != 39000 or audit["unique_scenarios"] != 39000:
        raise RuntimeError("D049 development grain does not match the frozen audit")
    fold_ids = sorted({str(row["fold_id"]) for row in fold_manifest_rows(phase_config, manifest)})
    if fold_ids != ["CV01", "CV02", "CV03", "CV04", "CV05"]:
        raise RuntimeError("D049 outer fold manifest is invalid")
    ...
    shared_gain = float(config["mathematical_contract"]["shared_first_stage_gain"])
    grid = tuple(float(value) for value in config["mathematical_contract"]["second_stage_gain_grid"])
    if shared_gain != 0.10 or grid != (0.05, 0.10, 0.15, 0.20):
        raise RuntimeError("D049 gain contract is not frozen")
    ...
    for outer_fold in fold_ids:
        ...
        for seed in seeds:
            ...
            candidate_gain, candidate_losses = _select_second_stage_gain(residual, shared_rbf25_oof, fidelity_oof, shared_gain, grid)
            control_gain, control_losses = _select_second_stage_gain(residual, shared_rbf25_oof, rbf50_oof, shared_gain, grid)
            selection_audit.append({"outer_fold": outer_fold, "seed_index": seed, "shared_gain": shared_gain, "grid": ";".join(f"{value:.2f}" for value in grid), "candidate_losses": ";".join(f"{value:.16g}" for value in candidate_losses), "control_losses": ";".join(f"{value:.16g}" for value in control_losses), "candidate_gain": candidate_gain, "control_gain": control_gain, "selection_source": "outer_training_inner_oof_only", "validation_outcomes_used": 0})
            ...
            first_stage = _fit_rbf_pair(aligned_train, residual, context.row_ids, "D049-q08-first", outer_fold, seed, 1.0, 256)
            first_validation = _predict_rbf_pair(first_stage, aligned_validation)[0.25]
            second_fidelity = _fit_fidelity(aligned_train, common_target, context.row_ids, "D049-q08-second", outer_fold, seed, 8, 1, 1.0, True, 1.0, 256)
            fidelity_validation = _predict_fidelity(second_fidelity, aligned_validation)
            second_rbf = _fit_rbf_pair(aligned_train, common_target, context.row_ids, "D049-q08-second", outer_fold, seed, 1.0, 256)
            rbf50_validation = _predict_rbf_pair(second_rbf, aligned_validation)[0.50]
            candidate_prediction = context.target_standardizer.inverse(c06_standardized_validation + shared_gain * first_validation + candidate_gain * fidelity_validation)
            control_prediction = context.target_standardizer.inverse(c06_standardized_validation + shared_gain * first_validation + control_gain * rbf50_validation)
            channel_audit.append({"outer_fold": outer_fold, "seed_index": seed, "q": 8, "shared_gain": shared_gain, "common_target_l2": float(np.linalg.norm(common_target)), "first_stage_rbf25_l2": float(np.linalg.norm(first_validation)), "second_stage_fidelity_l2": float(np.linalg.norm(fidelity_validation)), "second_stage_rbf50_l2": float(np.linalg.norm(rbf50_validation)), "candidate_gain": candidate_gain, "control_gain": control_gain, "validation_outcomes_used": 0, "stage_order": "fixed_shared_gain_then_nested_second_stage_gain", "channel_source": "inner_oof_selection_then_outer_refit"})
            for model_id, prediction in (("SSORFRK-08", candidate_prediction), ("SSO-TWO-RBF-08", control_prediction)):
    ...
    primary_id = "SSORFRK-08"
    control_id = "SSO-TWO-RBF-08"
    ...
    if c06_nrmse.size != 20 or primary_nrmse.size != 20 or control_nrmse.size != 20:
        raise RuntimeError("D049 paired seed endpoint is incomplete")
    interval = paired_bootstrap_mean_interval(primary_nrmse - c06_nrmse, confidence_level=0.95, replicates=10000, seed=49049)
    ...
    checks = {"five_percent_nrmse_improvement": primary_summary["mean_nrmse"] <= 0.95 * c06_summary["mean_nrmse"], "paired_upper_bound_below_zero": interval.upper < 0.0, "regret_not_worse": primary_summary["mean_mean_regret_m_s"] <= c06_summary["mean_mean_regret_m_s"], "infeasible_selection_not_worse": primary_summary["mean_infeasible_selection_rate"] <= c06_summary["mean_infeasible_selection_rate"], "beats_classical_stage_separated_control_by_five_percent": primary_summary["mean_nrmse"] <= 0.95 * control_summary["mean_nrmse"]}
    result = {"schema_version": "0.1.0", "decision_id": "D049", "protocol_id": "P017", "status": "complete_scientifically_superior" if all(checks.values()) else "complete_valid_negative", "source_commit": source_commit, "development_audit": audit, "development_rows_read": len(records), "calibration_rows_read": 0, "final_test_rows_read": 0, "hardware_jobs_submitted": 0, "gate6_runs": 0, "primary_model": primary_id, "primary_summary": primary_summary, "c06_summary": c06_summary, "classical_orthogonalized_summary": control_summary, "decision_checks": checks, "paired_nrmse_interval": {"estimate": interval.estimate, "lower": interval.lower, "upper": interval.upper, "confidence_level": interval.confidence_level}, "inner_audit_rows": len(inner_audit), "shared_stage_audit_rows": len(shared_stage_audit), "orthogonal_audit_rows": len(orthogonal_audit), "channel_audit_rows": len(channel_audit), "selection_audit_rows": len(selection_audit), "q": 8, "shared_first_stage_gain": shared_gain, "second_stage_gain_grid": list(grid), "tie_rule": "smallest grid value", "stage_order": "shared q8 RBF-0.25 fixed at 0.10; common e2=e-r25_q8_OOF; nested second-stage gain", "runtime": {"wall_clock_seconds": time.perf_counter() - started, "cpu_seconds": time.process_time() - cpu_started, "peak_working_set_gib": process_memory_observation().peak_bytes / 1024**3, "free_disk_gib": shutil.disk_usage(ROOT).free / 1024**3, "free_disk_preflight_gib": free_disk}, "claim_boundary": config["reporting"]["claim_boundary"], "result_files": ["inner_fold_audit.csv", "shared_stage_audit.csv", "orthogonal_audit.csv", "channel_audit.csv", "selection_audit.csv", "fold_seed_metrics.csv", "per_seed_metrics.csv", "summary.csv", "paired_comparison.csv"]}
