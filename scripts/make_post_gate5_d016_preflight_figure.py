"""Generate RFIG-033 for D016-C classical-first compute admission."""
...
RESULT = ROOT / "data/processed/reporting/post_gate5_d016_classical_preflight.json"
...
plt.rcParams.update(
    {
        ...
        "svg.hashsalt": "qmlforartemisiv-d016-preflight-v1",
    }
)
...
def _load_result() -> dict[str, Any]:
    ...
    if payload["decision_id"] != "D016-C":
        raise ValueError("RFIG-033 requires D016-C evidence")
    if payload["status"] not in {"PASS", "STOP"}:
        raise ValueError("RFIG-033 requires terminal D016-C evidence")
    ...
    for field in (
        ...
        if int(integrity[field]) != 0:
            raise PermissionError(f"D016-C figure source violates {field}")
    return payload
...
def _save(fig: plt.Figure, stem: str) -> tuple[Path, Path]:
    ...
    fig.savefig(
        ...
        dpi=300,
        bbox_inches="tight",
        metadata={"Software": "QMLforArtemisIV D016 preflight figure generator"},
    )
    fig.savefig(
        svg,
        bbox_inches="tight",
        metadata={
            "Creator": "QMLforArtemisIV D016 preflight figure generator",
            "Date": None,
        },
    )
    plt.close(fig)
...
def _draw(payload: dict[str, Any]) -> tuple[Path, Path]:
    ...
    ax.set_title(
        "D016-C synthetic compute admission for CRES/CSAFE",
        loc="left",
        fontsize=15,
        fontweight="bold",
        color=INK,
    ...
    ax.text(
        ...
        1.03,
        (
            "Clean-source synthetic evidence only: projects the largest-fold "
            "classical-first scaffold workload across 5 folds and 20 seeds."
        ),
    ...
    return _save(fig, "post_gate5_d016_classical_compute_admission")
...
def _register(png: Path, svg: Path, payload: dict[str, Any]) -> None:
    ...
    row.update(
        {
            "figure_id": "RFIG-033",
            "title": "D016-C classical-first synthetic compute admission",
            "phase": "Post-Gate-5 classical-first planning",
            "paper_section": "Methods: compute admission",
            "evidence_status": "classical_first_synthetic_compute_admission",
            "source_data": RESULT.relative_to(ROOT).as_posix(),
            "generator": "scripts/make_post_gate5_d016_preflight_figure.py",
            ...
            "caption": (
                "D016-C projects the clean-source synthetic CRES/CSAFE largest-fold "
                "workload across the frozen fold and seed count against the local "
                "laptop compute limits."
            ),
            "claim_boundary": (
                "Synthetic compute-admission evidence only; no development-data "
                "fitting, calibration/final-test access, hardware/GPU execution, "
                "Gate 5 reinterpretation, QML invention claim, quantum-advantage "
                "claim, or Gate 6."
            ),
...
def main() -> None:
    ...
    print("Generated RFIG-033 D016-C classical-first compute admission")
