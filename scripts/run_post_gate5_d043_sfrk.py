"""Run the D043 cross-fitted residual fidelity-RBF stack campaign."""
...
CONFIG_PATH = ROOT / "configs/post_gate5_d043_sfrk.yaml"
PHASE_CONFIG_PATH = ROOT / "configs/phase1_benchmark.yaml"


def _bounded_weight(first: np.ndarray, second: np.ndarray, target: np.ndarray) -> tuple[float, float, float]:
    """Fit the declared one-dimensional convex stack weight."""

    first_values = np.asarray(first, dtype=float)
    second_values = np.asarray(second, dtype=float)
    target_values = np.asarray(target, dtype=float)
    if first_values.shape != second_values.shape or first_values.shape != target_values.shape:
        raise ValueError("stack expert and target shapes do not match")
    if not np.all(np.isfinite(first_values)) or not np.all(np.isfinite(second_values)) or not np.all(np.isfinite(target_values)):
        raise ValueError("stack expert values must be finite")
    difference = first_values - second_values
    numerator = float(np.dot(difference, target_values - second_values))
    denominator = float(np.dot(difference, difference))
    if denominator <= 1.0e-18:
        return 0.5, numerator, denominator
    return float(np.clip(numerator / denominator, 0.0, 1.0)), numerator, denominator


def _inner_channel_predictions(
    ...
    """Generate inner OOF predictions for the three declared residual experts."""
    ...
    rbf25_oof = np.empty(raw.shape[0], dtype=float)
    ...
    for inner_fold in range(1, 5):
        ...
        if not hold_positions.size or not fit_positions.size:
            raise RuntimeError("D043 inner grouped fold is empty")
        ...
        if set(fit_groups) & set(hold_groups):
            raise RuntimeError("D043 inner channel fit/holdout groups overlap")
        ...
        inner_projection = TaskAlignedProjection(n_components=q).fit(fit_input, residual[fit_positions])
        aligned_fit = inner_projection.transform(fit_input)
        aligned_hold = inner_projection.transform(hold_input)
        row_ids = [str(context.row_ids[position]) for position in fit_positions]
        channel_id = f"D043-q{q:02d}-I{inner_fold:02d}"
        fidelity = _fit_fidelity(
            aligned_fit,
            residual[fit_positions],
            row_ids,
            channel_id,
            outer_fold,
            seed,
        ...
        rbf_predictions = _predict_rbf_pair(rbf_pair, aligned_hold)
        rbf25_oof[hold_positions] = rbf_predictions[0.25]
        rbf50_oof[hold_positions] = rbf_predictions[0.50]
        audit_rows.append(
            {
                ...
                "channel_source": "inner_fit_only_cross_fitted_c06_residual",
                "validation_outcomes_used": 0,
            }
        )
    if not np.all(np.isfinite(fidelity_oof)) or not np.all(np.isfinite(rbf25_oof)) or not np.all(np.isfinite(rbf50_oof)):
        raise RuntimeError("D043 inner channel OOF predictions are incomplete")
    return fidelity_oof, rbf25_oof, rbf50_oof, audit_rows
...
def run() -> dict[str, Any]:
    ...
    if config.get("decision_id") != "D043" or config.get("protocol_id") != "P011":
        raise RuntimeError("D043 config identity is invalid")
    if not bool(config["authority"]["research_data_fitting_authorized"]):
        raise RuntimeError("D043 fitting is not authorized")
    d042_result = json.loads(
        (ROOT / "data/processed/reporting/post_gate5_d042_agefrk/campaign_result.json").read_text(encoding="utf-8")
    )
    if d042_result.get("status") != "complete_valid_negative":
        raise RuntimeError("D043 requires an integrity-valid D042 negative")
    free_disk = shutil.disk_usage(ROOT).free / 1024**3
    floor = float(config["resources"]["free_disk_floor_gib"])
    if free_disk < floor:
        raise RuntimeError(f"D043 free disk preflight is below {floor:.1f} GiB: {free_disk:.3f} GiB")
    ...
    if result_path.is_file():
        ...
        if result.get("source_commit") != source_commit:
            raise RuntimeError("D043 result belongs to another source commit")
        return result
    ...
    if audit["rows"] != 39000 or audit["unique_scenarios"] != 39000:
        raise RuntimeError("D043 development grain does not match the frozen audit")
    fold_ids = sorted({str(row["fold_id"]) for row in fold_manifest_rows(phase_config, manifest)})
    if fold_ids != ["CV01", "CV02", "CV03", "CV04", "CV05"]:
        raise RuntimeError("D043 outer fold manifest is invalid")
    ...
    for outer_fold in fold_ids:
        ...
        for seed in seeds:
            ...
            for q in q_values:
                fidelity_oof, rbf25_oof, rbf50_oof, stack_rows = _inner_channel_predictions(
                    records, context, outer_fold, seed, q, residual, crossfit_baseline
                )
                stack_audit.extend(stack_rows)
                candidate_weight, candidate_numerator, candidate_denominator = _bounded_weight(fidelity_oof, rbf25_oof, residual)
                control_weight, control_numerator, control_denominator = _bounded_weight(rbf25_oof, rbf50_oof, residual)
                if not 0.0 <= candidate_weight <= 1.0 or not 0.0 <= control_weight <= 1.0:
                    raise RuntimeError("D043 stack weight escaped [0, 1]")
                ...
                if train_input.shape != (raw_train.shape[0], q + 1) or validation_input.shape != (raw_validation.shape[0], q + 1):
                    raise RuntimeError("D043 conditioning feature shape is invalid")
                if not np.all(np.isfinite(train_input)) or not np.all(np.isfinite(validation_input)):
                    raise RuntimeError("D043 conditioning features are not finite")
                ...
                candidate_id = f"SFRK-{q:02d}-CV"
                control_id = f"TWO-RBF-{q:02d}-CV"
                stack_audit.append(
                    {
                        "outer_fold": outer_fold,
                        "seed_index": seed,
                        ...
                        "fit_rows": int(residual.size),
                        "holdout_rows": int(residual.size),
                        ...
                        "weight_source": "inner_grouped_out_of_fold_training_predictions",
                        "validation_outcomes_used": 0,
                    }
                )
                ...
                candidate_delta = candidate_weight * fidelity_delta + (1.0 - candidate_weight) * rbf25_delta
                control_delta = control_weight * rbf25_delta + (1.0 - control_weight) * rbf50_delta
                ...
                for model_id, prediction in ((candidate_id, candidate_prediction), (control_id, control_prediction)):
    ...
    if c06_nrmse.size != 20 or primary_nrmse.size != 20 or control_nrmse.size != 20:
        raise RuntimeError("D043 paired seed endpoint is incomplete")
    interval = paired_bootstrap_mean_interval(primary_nrmse - c06_nrmse, confidence_level=0.95, replicates=10000, seed=43043)
    ...
    checks = {
        ...
        "beats_classical_stack_by_five_percent": primary_summary["mean_nrmse"] <= 0.95 * control_summary["mean_nrmse"],
    }
    result = {
        ...
        "classical_stack_summary": control_summary,
        ...
        "stack_audit_rows": len(stack_audit),
        "channel_audit_rows": len(channel_audit),
        "q_values": q_values,
        "rbf_gamma_multipliers": [0.25, 0.50],
        "weight_rule": "bounded convex weight from inner grouped out-of-fold training predictions",
        "runtime": {
        ...
        "result_files": ["inner_fold_audit.csv", "stack_audit.csv", "channel_audit.csv", "fold_seed_metrics.csv", "per_seed_metrics.csv", "summary.csv", "paired_comparison.csv"],
    }
    ...
    _write_csv(output_root / "paired_comparison.csv", [{"comparison": "SFRK-08-CV_minus_C06_nrmse", "seed_count": 20, "mean_difference": interval.estimate, "lower_95": interval.lower, "upper_95": interval.upper}])
