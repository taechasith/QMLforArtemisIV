"""Run the D039 error-conditioned global-fidelity residual-kernel campaign."""
...
try:
    from scripts.run_post_gate5_d038_gfrk import (  # noqa: E402
        _fit_fidelity,
        _predict_fidelity,
        _source_commit,
        _write_csv,
    ...
    if error.name != "scripts":
        raise
    from run_post_gate5_d038_gfrk import (  # type: ignore[no-redef] # noqa: E402
        _fit_fidelity,
        _predict_fidelity,
        _source_commit,
        _write_csv,
...
CONFIG_PATH = ROOT / "configs/post_gate5_d039_ecgfrk.yaml"
...
def _inner_cross_fitted_c06(
    ...
    for inner_fold in range(1, inner_folds + 1):
        ...
        if not hold_positions.size or not fit_positions.size:
            raise RuntimeError("D039 inner grouped fold is empty")
        ...
        if set(fit_groups) & set(hold_groups):
            raise RuntimeError("D039 inner C06 fit/holdout groups overlap")
        model_seed = int.from_bytes(
            hashlib.sha256(f"P007|D039|{outer_fold}|{inner_fold}|{seed}".encode()).digest()[-4:],
            "big",
        )
    ...
    if not np.all(np.isfinite(oof)):
        raise RuntimeError("D039 inner cross-fitted C06 residual is incomplete")
...
def run() -> dict[str, Any]:
    ...
    if config.get("decision_id") != "D039" or config.get("protocol_id") != "P007":
        raise RuntimeError("D039 config identity is invalid")
    if not bool(config["authority"]["research_data_fitting_authorized"]):
        raise RuntimeError("D039 fitting is not authorized")
    d038_result = json.loads(
        (ROOT / "data/processed/reporting/post_gate5_d038_gfrk/campaign_result.json").read_text(encoding="utf-8")
    )
    if d038_result.get("status") != "complete_valid_negative":
        raise RuntimeError("D039 requires an integrity-valid D038 negative")
    ...
    if result_path.is_file():
        ...
        if result.get("source_commit") != source_commit:
            raise RuntimeError("D039 result belongs to another source commit")
        return result
    ...
    if audit["rows"] != 39000 or audit["unique_scenarios"] != 39000:
        raise RuntimeError("D039 development grain does not match the frozen audit")
    fold_ids = sorted({str(row["fold_id"]) for row in fold_manifest_rows(phase_config, manifest)})
    if fold_ids != ["CV01", "CV02", "CV03", "CV04", "CV05"]:
        raise RuntimeError("D039 outer fold manifest is invalid")
    ...
    for outer_fold in fold_ids:
        ...
        for seed in seeds:
            ...
            for spec in specs:
                ...
                if train_input.shape != (raw_train.shape[0], q + 1):
                    raise RuntimeError("D039 training conditioning feature shape is invalid")
                if validation_input.shape != (raw_validation.shape[0], q + 1):
                    raise RuntimeError("D039 validation conditioning feature shape is invalid")
                if not np.all(np.isfinite(train_input)) or not np.all(np.isfinite(validation_input)):
                    raise RuntimeError("D039 conditioning features are not finite")
                ...
                projection_audit.append(
                    {
                        ...
                        "train_conditioning_hash": _feature_hash(crossfit_baseline),
                        "validation_conditioning_hash": _feature_hash(c06_standardized_validation),
                ...
                fidelity = _fit_fidelity(
                    ...
                    q,
                    int(spec["layers"]),
                    float(spec["feature_scale"]),
                    bool(spec["entangle"]),
                    float(spec["alpha"]),
                    int(spec["landmarks"]),
                )
                quantum_delta = _predict_fidelity(fidelity, aligned_validation)
                rbf = _fit_rbf(
                    aligned_train,
                    residual,
                    context.row_ids,
                ...
                rbf_id = f"EC-TAP-RBF-SHR-q{q}-L010"
                quantum_prediction = context.target_standardizer.inverse(
                    c06_standardized_validation + lambda_value * quantum_delta
                )
                rbf_prediction = context.target_standardizer.inverse(
                    c06_standardized_validation + lambda_value * rbf_delta
                )
    ...
    primary = "EC-GFRK-08-L010"
    control = "EC-TAP-RBF-SHR-q8-L010"
    ...
    if c06_nrmse.size != 20 or primary_nrmse.size != 20 or control_nrmse.size != 20:
        raise RuntimeError("D039 paired seed endpoint is incomplete")
    interval = paired_bootstrap_mean_interval(
        primary_nrmse - c06_nrmse, confidence_level=0.95, replicates=10000, seed=39039
    )
    ...
    result = {
        ...
        "result_files": ["inner_fold_audit.csv", "projection_audit.csv", "fidelity_audit.csv", "fold_seed_metrics.csv", "per_seed_metrics.csv", "summary.csv", "paired_comparison.csv"],
    }
    ...
    _write_csv(output_root / "paired_comparison.csv", [{"comparison": "EC-GFRK-08-L010_minus_C06_nrmse", "seed_count": 20, "mean_difference": interval.estimate, "lower_95": interval.lower, "upper_95": interval.upper}])
