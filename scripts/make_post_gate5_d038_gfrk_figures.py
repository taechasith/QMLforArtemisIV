"""Generate paper-ready figures for the D038 GFRK negative result."""
...
RESULT_ROOT = ROOT / "data/processed/reporting/post_gate5_d038_gfrk"
...
def _figure_comparison(summary: dict[str, dict[str, str]]) -> tuple[Path, Path]:
    ...
    labels = [model.replace("TAP-RBF-SHR-", "RBF-").replace("GFRK-", "G-") for model in models]
    colors = [
    ...
    fig.suptitle("D038 GFRK development-only result: global fidelity improved C06 but not the matched classical control", fontsize=14, fontweight="bold")
    fig.text(0.01, 0.01, "Five grouped outer folds; four inner whole-group folds; 20 seeds; 39,000 development rows. Lambda=0.10 was fixed before fitting.", fontsize=9)
    return _save(fig, "post_gate5_d038_gfrk_result_comparison")
...
def _figure_audit_resources(summary: dict[str, dict[str, str]]) -> tuple[Path, Path]:
    ...
    axes[0].set_xticks([1], ["GFRK-08-L010 minus C06"])
    ...
    axes[1].bar([str(q) for q in q_values], clipped, color="#e07a5f")
    axes[1].set_title("Global-fidelity PSD audit")
    axes[1].set_xlabel("Qubits")
    axes[1].set_ylabel("Mean eigenvalues clipped")
    ...
    fig.suptitle("D038 global-fidelity uncertainty, PSD audit, and compute boundary", fontsize=14, fontweight="bold")
    ...
    return _save(fig, "post_gate5_d038_gfrk_audit_resources")
...
def main() -> None:
    ...
    source_data = ";".join(
        [
            "data/processed/reporting/post_gate5_d038_gfrk/campaign_result.json",
            "data/processed/reporting/post_gate5_d038_gfrk/summary.csv",
            "data/processed/reporting/post_gate5_d038_gfrk/per_seed_metrics.csv",
            "data/processed/reporting/post_gate5_d038_gfrk/paired_comparison.csv",
            "data/processed/reporting/post_gate5_d038_gfrk/inner_fold_audit.csv",
            "data/processed/reporting/post_gate5_d038_gfrk/projection_audit.csv",
            "data/processed/reporting/post_gate5_d038_gfrk/fidelity_audit.csv",
        ]
    )
    generator = "scripts/make_post_gate5_d038_gfrk_figures.py"
    generator_hash = _sha256(ROOT / generator)
    definitions = [
        (
            "RFIG-062",
            "D038 GFRK result comparison",
            "Results: development-only negative",
            "d038_gfrk_result_comparison",
            "development_only_negative",
            "Global state fidelity improved C06 on the development endpoint but did not satisfy the 5% rule or beat the matched TAP-RBF control by 5%.",
        ...
        (
            "RFIG-063",
            "D038 GFRK paired audit and resources",
            "Methods and Results: fidelity audit and compute",
            "d038_gfrk_audit_resources",
            "development_only_negative",
            "The GFRK-08-L010 paired interval is below zero, while PSD, projection, and inner-fold audits are complete within the recorded resource envelope.",
            "Development-only uncertainty, PSD, integrity, and compute evidence; no NASA performance claim, mission claim, calibration/final-test result, hardware/GPU, Gate 5 revision, Gate 6, or quantum advantage.",
        ),
    ]
