"""Run the D042 adaptive-gated fidelity-RBF development campaign."""
...
CONFIG_PATH = ROOT / "configs/post_gate5_d042_agefrk.yaml"
PHASE_CONFIG_PATH = ROOT / "configs/phase1_benchmark.yaml"


def _adaptive_eta(probability: np.ndarray, eta_min: float = 0.25, eta_max: float = 0.75) -> np.ndarray:
    """Apply the frozen outcome-blind C06-probability gate."""

    values = np.asarray(probability, dtype=float)
    if not np.all(np.isfinite(values)) or np.any(values < 0.0) or np.any(values > 1.0):
        raise ValueError("C06 probabilities must be finite and in [0, 1]")
    if not 0.0 <= eta_min <= eta_max <= 1.0:
        raise ValueError("adaptive eta bounds are invalid")
    return eta_min + (eta_max - eta_min) * (1.0 - values)
...
def run() -> dict[str, Any]:
    ...
    if config.get("decision_id") != "D042" or config.get("protocol_id") != "P010":
        raise RuntimeError("D042 config identity is invalid")
    if not bool(config["authority"]["research_data_fitting_authorized"]):
        raise RuntimeError("D042 fitting is not authorized")
    d041_result = json.loads(
        (ROOT / "data/processed/reporting/post_gate5_d041_hefrk/campaign_result.json").read_text(encoding="utf-8")
    )
    if d041_result.get("status") != "complete_valid_negative":
        raise RuntimeError("D042 requires an integrity-valid D041 negative")
    free_disk = shutil.disk_usage(ROOT).free / 1024**3
    floor = float(config["resources"]["free_disk_floor_gib"])
    if free_disk < floor:
        raise RuntimeError(f"D042 free disk preflight is below {floor:.1f} GiB: {free_disk:.3f} GiB")
    ...
    if result_path.is_file():
        ...
        if result.get("source_commit") != source_commit:
            raise RuntimeError("D042 result belongs to another source commit")
        return result
    ...
    if audit["rows"] != 39000 or audit["unique_scenarios"] != 39000:
        raise RuntimeError("D042 development grain does not match the frozen audit")
    fold_ids = sorted({str(row["fold_id"]) for row in fold_manifest_rows(phase_config, manifest)})
    if fold_ids != ["CV01", "CV02", "CV03", "CV04", "CV05"]:
        raise RuntimeError("D042 outer fold manifest is invalid")
    ...
    candidate_specs = [dict(value) for value in config["candidate"]["configurations"]]
    q_values = sorted({int(spec["qubits"]) for spec in candidate_specs})
    gate = config["candidate"]["gate"]
    eta_min = float(gate["eta_min"])
    eta_max = float(gate["eta_max"])
    ...
    for outer_fold in fold_ids:
        ...
        for seed in seeds:
            ...
            eta_values = _adaptive_eta(probability, eta_min, eta_max)
            if eta_values.shape != probability.shape or not np.all(np.isfinite(eta_values)):
                raise RuntimeError("D042 adaptive gate is not finite")
            if np.any(eta_values < eta_min) or np.any(eta_values > eta_max):
                raise RuntimeError("D042 adaptive gate escaped its frozen bounds")
            gate_audit.append(
                {
                    "outer_fold": outer_fold,
                    "seed_index": seed,
                    "validation_rows": int(eta_values.size),
                    "eta_min": float(np.min(eta_values)),
                    "eta_max": float(np.max(eta_values)),
                    "eta_mean": float(np.mean(eta_values)),
                    "c06_probability_min": float(np.min(probability)),
                    "c06_probability_max": float(np.max(probability)),
                    "gate_formula": "0.25 + 0.50 * (1 - p_C06)",
                    "gate_source": "outer_fold_c06_feasibility_probability",
            ...
            for q in q_values:
                ...
                if train_input.shape != (raw_train.shape[0], q + 1) or validation_input.shape != (raw_validation.shape[0], q + 1):
                    raise RuntimeError("D042 conditioning feature shape is invalid")
                if not np.all(np.isfinite(train_input)) or not np.all(np.isfinite(validation_input)):
                    raise RuntimeError("D042 conditioning features are not finite")
                ...
                candidate_id = f"AGEFRK-{q:02d}-ADAPT"
                control_id = f"TWO-RBF-{q:02d}-ADAPT"
                ...
                channel_audit.append(
                    {
                        ...
                        "gate_eta_min": float(np.min(eta_values)),
                        "gate_eta_max": float(np.max(eta_values)),
                        "gate_eta_mean": float(np.mean(eta_values)),
                ...
                hybrid_delta = eta_values * fidelity_delta + (1.0 - eta_values) * rbf25_delta
                classical_delta = eta_values * rbf25_delta + (1.0 - eta_values) * rbf50_delta
                ...
                for model_id, prediction in ((candidate_id, quantum_prediction), (control_id, classical_prediction)):
                    ...
                    fold_metrics.append({"model_id": model_id, "config_id": model_id, "outer_fold": outer_fold, "seed_index": seed, "gate_eta_mean": float(np.mean(eta_values)), **_score(decision_ids, observed, feasible, prediction, probability, scale)})
    ...
    primary = "AGEFRK-08-ADAPT"
    control = "TWO-RBF-08-ADAPT"
    ...
    if c06_nrmse.size != 20 or primary_nrmse.size != 20 or control_nrmse.size != 20:
        raise RuntimeError("D042 paired seed endpoint is incomplete")
    interval = paired_bootstrap_mean_interval(primary_nrmse - c06_nrmse, confidence_level=0.95, replicates=10000, seed=42042)
    ...
    checks = {
        ...
        "beats_classical_adaptive_rbf_by_five_percent": primary_summary["mean_nrmse"] <= 0.95 * control_summary["mean_nrmse"],
    }
    result = {
        ...
        "classical_adaptive_rbf_summary": control_summary,
        ...
        "gate_audit_rows": len(gate_audit),
        "q_values": q_values,
        ...
        "result_files": ["inner_fold_audit.csv", "projection_audit.csv", "channel_audit.csv", "gate_audit.csv", "fold_seed_metrics.csv", "per_seed_metrics.csv", "summary.csv", "paired_comparison.csv"],
    }
    ...
    _write_csv(output_root / "gate_audit.csv", gate_audit)
    ...
    _write_csv(output_root / "paired_comparison.csv", [{"comparison": "AGEFRK-08-ADAPT_minus_C06_nrmse", "seed_count": 20, "mean_difference": interval.estimate, "lower_95": interval.lower, "upper_95": interval.upper}])
