"""Run the D038 global-fidelity residual-kernel campaign."""
...
CONFIG_PATH = ROOT / "configs/post_gate5_d038_gfrk.yaml"
...
def _inner_cross_fitted_c06(
    ...
    for inner_fold in range(1, inner_folds + 1):
        ...
        if not hold_positions.size or not fit_positions.size:
            raise RuntimeError("D038 inner grouped fold is empty")
        ...
        if set(fit_groups) & set(hold_groups):
            raise RuntimeError("D038 inner C06 fit/holdout groups overlap")
        model_seed = int.from_bytes(
            hashlib.sha256(f"P006|D038|{outer_fold}|{inner_fold}|{seed}".encode()).digest()[-4:],
            "big",
        )
    ...
    if not np.all(np.isfinite(oof)):
        raise RuntimeError("D038 inner cross-fitted C06 residual is incomplete")
...
def _fit_fidelity(
    ...
    if alpha <= 0.0 or landmarks <= 0:
        raise ValueError("D038 fidelity ridge settings must be positive")
    ...
    coefficients = np.linalg.solve(
        embedded.T @ embedded + alpha * np.eye(embedded.shape[1]),
        embedded.T @ target,
    )
    return {
...
def _predict_fidelity(model: Mapping[str, Any], values: np.ndarray) -> np.ndarray:
...
def run() -> dict[str, Any]:
    ...
    if config.get("decision_id") != "D038" or config.get("protocol_id") != "P006":
        raise RuntimeError("D038 config identity is invalid")
    if not bool(config["authority"]["research_data_fitting_authorized"]):
        raise RuntimeError("D038 fitting is not authorized")
    d037_result = json.loads(
        (ROOT / "data/processed/reporting/post_gate5_d037_tsqr/campaign_result.json").read_text(encoding="utf-8")
    )
    if d037_result.get("status") != "complete_valid_negative":
        raise RuntimeError("D038 requires an integrity-valid D037 negative")
    ...
    if result_path.is_file():
        ...
        if result.get("source_commit") != source_commit:
            raise RuntimeError("D038 result belongs to another source commit")
        return result
    ...
    if audit["rows"] != 39000 or audit["unique_scenarios"] != 39000:
        raise RuntimeError("D038 development grain does not match the frozen audit")
    fold_ids = sorted({str(row["fold_id"]) for row in fold_manifest_rows(phase_config, manifest)})
    if fold_ids != ["CV01", "CV02", "CV03", "CV04", "CV05"]:
        raise RuntimeError("D038 outer fold manifest is invalid")
    ...
    primary = "GFRK-08-L010"
    control = "TAP-RBF-SHR-q8-L010"
    ...
    if c06_nrmse.size != 20 or primary_nrmse.size != 20 or control_nrmse.size != 20:
        raise RuntimeError("D038 paired seed endpoint is incomplete")
    interval = paired_bootstrap_mean_interval(
        primary_nrmse - c06_nrmse, confidence_level=0.95, replicates=10000, seed=38038
    )
    ...
    _write_csv(output_root / "paired_comparison.csv", [{"comparison": "GFRK-08-L010_minus_C06_nrmse", "seed_count": 20, "mean_difference": interval.estimate, "lower_95": interval.lower, "upper_95": interval.upper}])
