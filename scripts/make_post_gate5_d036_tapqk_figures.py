"""Generate paper-ready figures for the D036 TAP-QK negative result."""
...
RESULT_ROOT = ROOT / "data/processed/reporting/post_gate5_d036_tapqk"
...
def _figure_comparison(summary: dict[str, dict[str, str]]) -> tuple[Path, Path]:
    ...
    labels = [model.replace("TAP-RBF-", "RBF-").replace("TAPQK-", "Q-") for model in models]
    colors = [
        "#1b4965" if model == "C06" else "#e07a5f" if model.startswith("TAP-RBF") else "#2a9d8f"
        for model in models
    ]
    ...
    fig.suptitle("D036 TAP-QK development-only result: task alignment did not improve C06", fontsize=14, fontweight="bold")
    fig.text(0.01, 0.01, "Five grouped outer folds; four inner whole-group folds; 20 seeds; 39,000 development rows. TAP-RBF is the identical-input classical control.", fontsize=9)
    return _save(fig, "post_gate5_d036_tapqk_result_comparison")
...
def _figure_audit_resources(summary: dict[str, dict[str, str]]) -> tuple[Path, Path]:
    ...
    assert len(projection_audit) == 5 * 20 * 3
    assert len(inner_audit) == 5 * 20 * 4
    ...
    axes[0].set_xticks([1], ["TAPQK-08 minus C06"])
    axes[0].set_ylabel("Seed-pooled NRMSE difference")
    axes[0].set_title("Paired endpoint evidence")
    axes[0].legend(fontsize=8)

    q_values = [4, 6, 8]
    counts = [sum(int(row["projection_components"]) == q for row in projection_audit) for q in q_values]
    axes[1].bar([str(q) for q in q_values], counts, color="#e07a5f")
    axes[1].set_title("Task-alignment audit")
    axes[1].set_xlabel("PLS components / qubits")
    axes[1].set_ylabel("Projection records")
    axes[1].text(0.02, 0.95, "validation outcomes used: 0\ninner group overlaps: 0", transform=axes[1].transAxes, va="top", fontsize=9)
    ...
    fig.suptitle("D036 leakage audit and compute boundary", fontsize=14, fontweight="bold")
    ...
    return _save(fig, "post_gate5_d036_tapqk_audit_resources")
...
def main() -> None:
    ...
    source_data = ";".join(
        [
            "data/processed/reporting/post_gate5_d036_tapqk/campaign_result.json",
            "data/processed/reporting/post_gate5_d036_tapqk/summary.csv",
            "data/processed/reporting/post_gate5_d036_tapqk/per_seed_metrics.csv",
            "data/processed/reporting/post_gate5_d036_tapqk/paired_comparison.csv",
            "data/processed/reporting/post_gate5_d036_tapqk/inner_fold_audit.csv",
            "data/processed/reporting/post_gate5_d036_tapqk/projection_audit.csv",
        ]
    )
    generator = "scripts/make_post_gate5_d036_tapqk_figures.py"
    generator_hash = _sha256(ROOT / generator)
    definitions = [
        (
            "RFIG-058",
            "D036 TAP-QK result comparison",
            "Results: development-only negative",
            "d036_result_comparison",
            "development_only_negative",
            "D036 TAP-QK did not beat the frozen C06 predictor or the identical-input TAP-RBF control.",
        ...
        (
            "RFIG-059",
            "D036 TAP-QK audit and resources",
            "Methods and Results: projection audit and compute",
            "d036_projection_audit_resources",
            "development_only_negative",
            "The paired TAPQK-08 minus C06 interval is entirely positive, while projection and inner-fold audits show zero validation-outcome use and zero group overlap.",
            "Development-only uncertainty, leakage, and compute evidence; no NASA performance claim, mission claim, calibration/final-test result, hardware/GPU, Gate 5 revision, Gate 6, or quantum advantage.",
        ),
    ]
