"""Run the D044 nonlinear interaction fidelity-RBF campaign."""
...
CONFIG_PATH = ROOT / "configs/post_gate5_d044_nifrk.yaml"
PHASE_CONFIG_PATH = ROOT / "configs/phase1_benchmark.yaml"


def _interaction_features(first: np.ndarray, second: np.ndarray) -> np.ndarray:
    ...
    if first_values.shape != second_values.shape:
        raise ValueError("interaction expert shapes do not match")
    features = np.column_stack(
        (
            np.ones(first_values.size),
            first_values,
            second_values,
    ...
    if not np.all(np.isfinite(features)):
        raise ValueError("interaction features are not finite")
    return features


def _fit_interaction(first: np.ndarray, second: np.ndarray, target: np.ndarray, alpha: float) -> np.ndarray:
    features = _interaction_features(first, second)
    values = np.asarray(target, dtype=float)
    if features.shape[0] != values.size or not np.all(np.isfinite(values)):
        raise ValueError("interaction target is invalid")
    regularizer = np.diag([0.0, alpha, alpha, alpha, alpha, alpha])
    coefficients = np.linalg.solve(features.T @ features + regularizer, features.T @ values)
    if not np.all(np.isfinite(coefficients)):
        raise RuntimeError("interaction coefficients are not finite")
    return coefficients
...
def run() -> dict[str, Any]:
    ...
    if config.get("decision_id") != "D044" or config.get("protocol_id") != "P012":
        raise RuntimeError("D044 config identity is invalid")
    if not bool(config["authority"]["research_data_fitting_authorized"]):
        raise RuntimeError("D044 fitting is not authorized")
    d043_result = json.loads(
        (ROOT / "data/processed/reporting/post_gate5_d043_sfrk/campaign_result.json").read_text(encoding="utf-8")
    )
    if d043_result.get("status") != "complete_valid_negative":
        raise RuntimeError("D044 requires an integrity-valid D043 negative")
    free_disk = shutil.disk_usage(ROOT).free / 1024**3
    floor = float(config["resources"]["free_disk_floor_gib"])
    if free_disk < floor:
        raise RuntimeError(f"D044 free disk preflight is below {floor:.1f} GiB: {free_disk:.3f} GiB")
    ...
    if result_path.is_file():
        ...
        if result.get("source_commit") != source_commit:
            raise RuntimeError("D044 result belongs to another source commit")
        return result
    ...
    if audit["rows"] != 39000 or audit["unique_scenarios"] != 39000:
        raise RuntimeError("D044 development grain does not match the frozen audit")
    fold_ids = sorted({str(row["fold_id"]) for row in fold_manifest_rows(phase_config, manifest)})
    if fold_ids != ["CV01", "CV02", "CV03", "CV04", "CV05"]:
        raise RuntimeError("D044 outer fold manifest is invalid")
    ...
    interaction_ridge = float(config["candidate"]["interaction_ridge"])
    ...
    for outer_fold in fold_ids:
        ...
        for seed in seeds:
            ...
            for q in q_values:
                fidelity_oof, rbf25_oof, rbf50_oof, channel_inner_rows = _inner_channel_predictions(
                    records, context, outer_fold, seed, q, residual, crossfit_baseline
                )
                interaction_audit.extend(channel_inner_rows)
                candidate_coefficients = _fit_interaction(fidelity_oof, rbf25_oof, residual, interaction_ridge)
                control_coefficients = _fit_interaction(rbf25_oof, rbf50_oof, residual, interaction_ridge)
                ...
                if train_input.shape != (raw_train.shape[0], q + 1) or validation_input.shape != (raw_validation.shape[0], q + 1):
                    raise RuntimeError("D044 conditioning feature shape is invalid")
                if not np.all(np.isfinite(train_input)) or not np.all(np.isfinite(validation_input)):
                    raise RuntimeError("D044 conditioning features are not finite")
                ...
                candidate_id = f"NIFRK-{q:02d}-NL"
                control_id = f"TWO-RBF-{q:02d}-NL"
                interaction_audit.append(
                    {
                        ...
                        "candidate_coefficients": ";".join(f"{value:.16g}" for value in candidate_coefficients),
                        "control_coefficients": ";".join(f"{value:.16g}" for value in control_coefficients),
                        "feature_map": "[1,u,v,u*v,u^2,v^2]",
                ...
                channel_audit.append(
                    {
                        ...
                        "candidate_interaction_norm": float(np.linalg.norm(candidate_coefficients[3:])),
                        "control_interaction_norm": float(np.linalg.norm(control_coefficients[3:])),
                ...
                candidate_delta = _interaction_features(fidelity_delta, rbf25_delta) @ candidate_coefficients
                control_delta = _interaction_features(rbf25_delta, rbf50_delta) @ control_coefficients
    ...
    if c06_nrmse.size != 20 or primary_nrmse.size != 20 or control_nrmse.size != 20:
        raise RuntimeError("D044 paired seed endpoint is incomplete")
    interval = paired_bootstrap_mean_interval(primary_nrmse - c06_nrmse, confidence_level=0.95, replicates=10000, seed=44044)
    ...
    checks = {
        ...
        "beats_classical_nonlinear_stack_by_five_percent": primary_summary["mean_nrmse"] <= 0.95 * control_summary["mean_nrmse"],
    }
    result = {
        ...
        "classical_nonlinear_summary": control_summary,
        ...
        "interaction_audit_rows": len(interaction_audit),
        "channel_audit_rows": len(channel_audit),
        "q_values": q_values,
        "interaction_feature_map": "[1,u,v,u*v,u^2,v^2]",
        "interaction_ridge": interaction_ridge,
        "runtime": {
        ...
        "result_files": ["inner_fold_audit.csv", "interaction_audit.csv", "channel_audit.csv", "fold_seed_metrics.csv", "per_seed_metrics.csv", "summary.csv", "paired_comparison.csv"],
    }
    _write_csv(output_root / "inner_fold_audit.csv", inner_audit)
    _write_csv(output_root / "interaction_audit.csv", interaction_audit)
    ...
    _write_csv(output_root / "paired_comparison.csv", [{"comparison": "NIFRK-08-NL_minus_C06_nrmse", "seed_count": 20, "mean_difference": interval.estimate, "lower_95": interval.lower, "upper_95": interval.upper}])
