"""Run the D036 task-aligned projected quantum-kernel campaign."""
...
CONFIG_PATH = ROOT / "configs/post_gate5_d036_tapqk.yaml"
...
def _source_commit() -> str:
    if _git("status", "--porcelain"):
        raise RuntimeError("D036 requires a clean source worktree")
    if _git("branch", "--show-current") != "main":
        raise RuntimeError("D036 requires the main branch")
...
def _fit_rbf(
    ...
    if positive.size == 0:
        raise RuntimeError("D036 TAP-RBF median distance is zero")
    ...
    landmarks_values = training[indices]

    def kernel(left: np.ndarray, right: np.ndarray) -> np.ndarray:
        return np.exp(-gamma * _squared_distances(left, right))

    landmark_kernel = kernel(landmarks_values, landmarks_values)
    ...
    embedded = kernel(training, landmarks_values) @ inverse_root
    coefficients = np.linalg.solve(
        embedded.T @ embedded + float(alpha) * np.eye(embedded.shape[1]),
        embedded.T @ target,
    )
    return {
        "landmarks": landmarks_values,
...
def _inner_cross_fitted_c06(
    ...
    for inner_fold in range(1, inner_folds + 1):
        ...
        if not hold_positions.size or not fit_positions.size:
            raise RuntimeError("D036 inner grouped fold is empty")
        ...
        if set(fit_groups) & set(hold_groups):
            raise RuntimeError("D036 inner C06 fit/holdout groups overlap")
        model_seed = int.from_bytes(
            hashlib.sha256(f"P004|D036|{outer_fold}|{inner_fold}|{seed}".encode()).digest()[-4:],
            "big",
        )
    ...
    if not np.all(np.isfinite(oof)):
        raise RuntimeError("D036 inner cross-fitted C06 residual is incomplete")
    return oof, audit_rows


def _projection_hash(projection: TaskAlignedProjection) -> str:
    digest = hashlib.sha256()
    for value in (
        projection.pls_.x_weights_,
        projection.pls_.x_loadings_,
        projection.score_mean_,
        projection.score_scale_,
    ):
...
def run() -> dict[str, Any]:
    ...
    if config.get("decision_id") != "D036" or config.get("protocol_id") != "P004":
        raise RuntimeError("D036 config identity is invalid")
    if not bool(config["authority"]["research_data_fitting_authorized"]):
        raise RuntimeError("D036 fitting is not authorized")
    d035_result = json.loads(
        (ROOT / "data/processed/reporting/post_gate5_d035_cfqsr/campaign_result.json").read_text(encoding="utf-8")
    )
    if d035_result.get("status") != "complete_valid_negative":
        raise RuntimeError("D036 requires an integrity-valid D035 negative")
    ...
    if result_path.is_file():
        ...
        if result.get("source_commit") != source_commit:
            raise RuntimeError("D036 result belongs to another source commit")
        return result
    ...
    if audit["rows"] != 39000 or audit["unique_scenarios"] != 39000:
        raise RuntimeError("D036 development grain does not match the frozen audit")
    fold_ids = sorted({str(row["fold_id"]) for row in fold_manifest_rows(phase_config, manifest)})
    if fold_ids != ["CV01", "CV02", "CV03", "CV04", "CV05"]:
        raise RuntimeError("D036 outer fold manifest is invalid")
        ...
        for seed in seeds:
            ...
            for spec in specs:
                ...
                quantum_prediction = context.target_standardizer.inverse(
                    c06_standardized_validation + quantum.predict(aligned_validation)
                )
                key = (config_id, seed)
                prediction_store.setdefault(key, {"cost": [], "probability": [], "labels": [], "observed": [], "decision_ids": []})
                ...
                rbf_prediction = context.target_standardizer.inverse(
                    c06_standardized_validation + _predict_rbf(rbf, aligned_validation)
                )
                rbf_id = f"TAP-RBF-q{q}"
                key = (rbf_id, seed)
                prediction_store.setdefault(key, {"cost": [], "probability": [], "labels": [], "observed": [], "decision_ids": []})
    ...
    rbf_nrmse = metric("TAP-RBF-q8", "nrmse")
    if c06_nrmse.size != 20 or primary_nrmse.size != 20 or rbf_nrmse.size != 20:
        raise RuntimeError("D036 paired seed endpoint is incomplete")
    interval = paired_bootstrap_mean_interval(
        primary_nrmse - c06_nrmse, confidence_level=0.95, replicates=10000, seed=36036
    )
    ...
    rbf_summary = next(row for row in summary if row["model_id"] == "TAP-RBF-q8")
    checks = {
        ...
        "beats_classical_tap_rbf_by_five_percent": primary_summary["mean_nrmse"] <= 0.95 * rbf_summary["mean_nrmse"],
    }
    result = {
        ...
        "tap_rbf_q8_summary": rbf_summary,
        ...
        "result_files": ["inner_fold_audit.csv", "projection_audit.csv", "fold_seed_metrics.csv", "per_seed_metrics.csv", "summary.csv", "paired_comparison.csv"],
    }
    ...
    _write_csv(output_root / "paired_comparison.csv", [{"comparison": "TAPQK-08_minus_C06_nrmse", "seed_count": 20, "mean_difference": interval.estimate, "lower_95": interval.lower, "upper_95": interval.upper}])
