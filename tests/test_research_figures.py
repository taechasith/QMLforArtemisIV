...
def test_figure_registry_has_unique_ids_and_matching_artifacts() -> None:
    ...
    rfig046 = next(row for row in rows if row["figure_id"] == "RFIG-046")
    assert rfig046["evidence_status"] == "manuscript_results_discussion_draft"
    assert "post_gate5_d027_manuscript_results.json" in rfig046["source_data"]
    assert rfig046["reporting_source_commit"] == (
        "dd4b8650947deb193ebf27699139a051991ae5c3"
    )
    assert rfig046["figure_generator_sha256"] == sha256_file(
        ROOT / rfig046["generator"]
    )
    rfig047 = next(row for row in rows if row["figure_id"] == "RFIG-047")
    assert rfig047["evidence_status"] == "release_support_cards_ready"
    assert "post_gate5_d028_release_support_cards.json" in rfig047["source_data"]
    assert rfig047["reporting_source_commit"] == (
        "4e9e184aab69504a3fbf6c085c1196abf1aca3b9"
    )
    assert rfig047["figure_generator_sha256"] == sha256_file(
        ROOT / rfig047["generator"]
    )
    rfig048 = next(row for row in rows if row["figure_id"] == "RFIG-048")
    assert rfig048["evidence_status"] == "clean_reproducibility_audit_stop"
    assert "post_gate5_d029_reproducibility_audit.json" in rfig048["source_data"]
    assert rfig048["reporting_source_commit"] == (
    ...
    rfig050 = next(row for row in rows if row["figure_id"] == "RFIG-050")
    assert rfig050["evidence_status"] == "claim_release_review_ready"
    assert "post_gate5_d031_claim_release_review.json" in rfig050["source_data"]
    assert rfig050["reporting_source_commit"] == (
        "90f45d356faa480998573cbc3b25b6e819b95ae8"
    )
    assert rfig050["figure_generator_sha256"] == sha256_file(
        ROOT / rfig050["generator"]
    )
    rfig051 = next(row for row in rows if row["figure_id"] == "RFIG-051")
    assert rfig051["evidence_status"] == "release_candidate_manifest_ready"
    assert "post_gate5_d032_release_candidate_manifest.json" in rfig051["source_data"]
    assert rfig051["reporting_source_commit"] == (
