...
D035 is a valid scientific negative. CFQSR-08-N achieved mean pooled OOF NRMSE
`0.0134629` versus C06 `0.00683281`; the paired CFQSR-minus-C06 difference was
`+0.00663006` with 95% interval `[+0.00662749, +0.00663245]`. A02-STACK-q8
scored `0.00984926`, so cross-fitting did not beat the matched classical
control. The endpoint completed all five outer folds, four inner folds, and 20
seeds from source commit `a16938d`, with 39,000 development rows and zero
calibration/final-test reads. RFIG-056 and RFIG-057 record the result,
cross-fitting audit, uncertainty, and resource boundary. D035 does not revise
Gate 5 or authorize Gate 6.

D036/P004 is the next prospective invention protocol opened from the D035
lesson. It tests Task-Aligned Projected Quantum Kernel (TAP-QK): a fold-local
PLS projection fit to cross-fitted C06 residuals, followed by the existing
projected quantum kernel. TAP-RBF uses the identical supervised coordinates,
landmarks, bandwidth, and ridge solve. D036 uses q=4/6/8, four inner grouped
...
D037 is a valid scientific negative. The primary TSQR-08-L025 scored mean
pooled OOF NRMSE `0.00684778` versus C06 `0.00683281`; the paired difference
was `+0.0000149672` with 95% interval `[+0.0000146573,+0.0000152701]`. The
best declared ablation TSQR-08-L010 scored `0.00675013`, but the matched
TAP-RBF-SHR-08-L010 scored `0.00675359`, so the modest gain was not
quantum-specific and did not meet the 5% primary rule. D037 completed 900
shrinkage audits with zero validation-outcome use, zero inner-group overlap,
39,000 development rows, and zero calibration/final-test reads. RFIG-060 and
RFIG-061 record the full lambda grid, uncertainty, audit, and resources.

D038/P006 is the next prospective invention protocol. It tests Global Fidelity
Residual Kernel (GFRK): exact full-state fidelity with one entangling layer,
retaining D036 task-aligned coordinates and fixed lambda=0.10. TAP-RBF-SHR-q
is the identical-input classical control. D038 is development-only and does
not revise Gate 5, D034-D037, or authorize Gate 6.

D038 is a valid scientific negative. GFRK-08-L010 scored mean pooled OOF NRMSE
`0.00664716` versus C06 `0.00683281`; the paired difference was
`-0.000185652` with 95% interval `[-0.000187118,-0.000184266]`. This is an
approximately 2.7% improvement against C06, below the frozen 5% rule, and it
did not beat TAP-RBF-SHR-q8-L010 by 5%. All five outer folds, four inner folds,
20 seeds, PSD/fidelity audits, and 39,000 development rows completed; no
validation outcomes were used, and calibration/final-test, hardware/GPU,
mission-loop, and Gate 6 counters remained zero. RFIG-062 and RFIG-063 are
the required paper figures. The improvement is a useful invention signal, not
a superiority claim.

D039/P007 is the next prospective invention protocol. It tests an
Error-Conditioned Global Fidelity Residual Kernel (EC-GFRK): the honest outer
C06 prediction is included as an input feature to the fold-local,
cross-fitted-residual PLS map before the same exact global fidelity kernel.
The matched classical control receives the identical error-conditioned scores,
landmarks, ridge, and lambda. D039 is development-only and does not revise
Gate 5 or D034-D038, and does not authorize Gate 6.

D039 is a valid scientific negative. EC-GFRK-08-L010 scored mean pooled OOF
NRMSE `0.00646644` versus C06 `0.00683281`, an approximately 5.36%
improvement, with paired difference `-0.000366372` and 95% interval
`[-0.000367832,-0.000365004]`. Regret and infeasible-selection conditions
were preserved, but the identical-input EC-TAP-RBF-SHR-q8 control scored
`0.00675401`, so the candidate did not beat its classical control by 5%.
The result is therefore a valid development-only negative, not a new QML
success. All five outer folds, four inner folds, 20 seeds, 39,000 development
rows, 300 projection audits, and 300 fidelity audits completed with zero
validation-outcome use, zero group overlap, and zero calibration/final-test,
hardware/GPU, mission-loop, and Gate 6 activity. RFIG-064 and RFIG-065 are
the required paper figures.

D040/P008 is the next prospective invention protocol. It tests a Centered
Error-Conditioned Global Fidelity Residual Kernel (CE-GFRK): center the
error-conditioned fidelity feature map using training rows only before the
same ridge correction. The matched classical control receives the identical
centered feature treatment. D040 is development-only and does not revise
Gate 5 or D034-D039, and does not authorize Gate 6.

D040 is a valid scientific negative. CE-GFRK-08-L010 scored mean pooled OOF
NRMSE `0.00650367` versus C06 `0.00683281`, with paired difference
`-0.000329145` and 95% interval `[-0.000330332,-0.000327932]`. This was about
4.82% below C06, so it missed the frozen 5% threshold; the centered classical
control scored `0.00677146` and no classical-specific superiority condition
passed. All five outer folds, four inner folds, 20 seeds, 39,000 development
rows, and 300 centering audits completed; centering means were training-only,
validation outcomes and group overlaps were zero, and calibration/final-test,
hardware/GPU, mission-loop, and Gate 6 counters remained zero. RFIG-066 and
RFIG-067 are the required paper figures. The result indicates that centering
did not improve the D039 representation.

D041/P009 is the next prospective invention protocol. It tests a fixed hybrid
mixture of the D039 error-conditioned global-fidelity correction and a
...
D044 is a valid scientific negative. NIFRK-08-NL scored mean pooled OOF NRMSE
`0.006751395` versus C06 `0.006832811`, with paired difference
`-0.000081416` and 95% interval `[-0.0000840,-0.0000787]`. This was only about
1.19% below C06, so the frozen 5% threshold failed. The matched nonlinear
TWO-RBF-08-NL control scored `0.006765864`; the candidate also failed the
classical-specific 5% condition. Regret and infeasible-selection conditions
were preserved. All 39,000 development rows, five outer folds, four inner
folds, 20 seeds, 300 channel audits, 1,500 interaction audits, and figures
RFIG-074/RFIG-075 completed with zero construction-time validation-outcome use,
zero group overlap, zero locked-data reads, and zero hardware/GPU, mission-loop,
and Gate 6 activity. The useful signal is that quadratic interaction terms
reduced the D043 gain and do not justify continuing the nonlinear branch.

D045/P013 is the next prospective invention protocol. It tests a fixed
multi-scale linear stack using fidelity channels at q=4, 6, and 8 together
with the RBF-0.25 channels. The matched control has the same six-channel
budget, replacing fidelity with RBF-0.50. Both stacks fit on inner grouped
out-of-fold training predictions. D045 is development-only and does not revise
Gate 5 or D034-D044, and does not authorize Gate 6.

D045 is a valid scientific negative. MSFRK-ALL scored mean pooled OOF NRMSE
`0.006731045` versus C06 `0.006832811`, with paired difference
`-0.000101766` and 95% interval `[-0.000103313,-0.000100332]`. The improvement
was about 1.49%, below the frozen 5% rule. The matched MS-TWO-RBF control
scored `0.006869509`, so the classical-specific 5% condition also failed.
Regret and infeasible-selection conditions were preserved. All 39,000
development rows, five outer folds, four inner folds, 20 seeds, 300 channel
audits, 1,300 multi-scale audits, and RFIG-076/RFIG-077 completed with zero
construction-time validation-outcome use, zero group overlap, zero locked-data
reads, and zero hardware/GPU, mission-loop, or Gate 6 activity. The useful
signal is that adding q=4 and q=6 fidelity channels did not recover the
classical-specific gap.

D046/P014 is the next prospective invention protocol. It tests an
orthogonalized residual correction: a shared RBF-0.25 first stage is fitted to
the C06 residual, then a second-stage expert is fitted to the residual left
after the inner out-of-fold RBF-0.25 prediction. The candidate uses a fidelity
second stage and the matched control uses an RBF-0.50 second stage. This
isolates information unique to fidelity after the common classical correction
and does not alter Gate 5, D034-D045, or Gate 6 authorization.

D046 is a valid scientific negative. ORFRK-08-R2 scored mean pooled OOF NRMSE
`0.006436255` versus C06 `0.006832811`, with paired difference
`-0.000396556` and 95% interval `[-0.000398722,-0.000394287]`. The improvement
was about 5.80%, so the C06 threshold passed, but the matched TWO-RBF-08-R2
control scored `0.006750081`, leaving about 4.65% candidate-over-control
improvement and failing the frozen classical-specific 5% rule. All 39,000
development rows, five outer folds, four inner folds, 20 seeds, 300 channel
audits, 1,200 orthogonalized inner audits, and RFIG-078/RFIG-079 completed
with zero validation-outcome use, zero group overlap, zero locked-data reads,
and zero hardware/GPU, mission-loop, or Gate 6 activity. The useful signal is
that orthogonalization exposes a stronger fidelity residual, but it remains
slightly short of the required classical-specific margin.

D047/P015 is the next prospective invention protocol. It keeps one shared
q=8 RBF-0.25 first-stage correction and fits a fixed three-channel second-stage
stack from q=4/6/8 fidelity predictions on the common orthogonalized residual.
The matched control uses the same stack with q=4/6/8 RBF-0.50 predictions.
This tests cross-q complementarity after shared-signal removal without
post-outcome q selection or a larger model family.

D047 is a valid scientific negative. OMFRK-ALL scored mean pooled OOF NRMSE
`0.006951944` versus C06 `0.006832811`, with paired difference `+0.000119133`
and 95% interval `[0.000113372,0.000125009]`. The candidate was worse than
C06 and the matched OM-TWO-RBF control scored `0.006804935`. The q=4/6/8
stack is not supported by this evidence; multi-scale stacking will not be
expanded further unless a later protocol has a separate falsifiable mechanism.
All 39,000 development rows, 400 shared-stage audits, 1,200 second-stage
audits, 100 stack audits, and RFIG-080/RFIG-081 completed with zero
validation-outcome use, zero group overlap, zero locked-data reads, and zero
hardware/GPU, mission-loop, or Gate 6 activity.

D048/P016 is the next prospective invention protocol. It returns to the D046
q=8 endpoint, retains the common RBF-0.25 first stage, and selects a small
predeclared shrinkage grid `{0.05, 0.10, 0.15}` from inner-training OOF
...
D053-C accepts the Stage 1 numerical-audit freeze. It requires a direct economy
SVD of the orthonormal Pauli/DLA coefficient commutator matrix, with the
relative rank threshold `max(C.shape) * eps_float64 * sigma_max`, full
singular-spectrum records, and a hard nullity-stability check at `0.1*tau`,
`tau`, and `10*tau`. Normal-equation eigensolves are prohibited. The audit also
stops rather than silently projecting through an unclosed DLA basis or
exceeding the 1,024-basis local resource ceiling.

The decoupled trajectory replay is fixed to the existing DOP853 implementation
under default and 100x-tighter/half-step settings. A fixed-context effect may
be called physical only when it is above the joint-covariance numerical floor
and stable within the preregistered 1 percent replay tolerance. D053-C creates
no centralizer, trajectory result, dataset, model fit, raw evidence, or figure.
The concrete representation, generators, rotations, target scale, strata, and
figure registry still require a separate prospective decision before one
bounded audit can be authorized. Gate 6 remains unauthorized.

D054-A accepts that binding decision without authorizing execution. It excludes
deterministic SRP, fixes Earth-centered J2000/ICRF-compatible axes and F0/F1/F2
force scope, and defines a q=4 collective-spin `SO(3)` representation with a
255-element Pauli `su(16)` basis. The direct-SVD audit must stop if the frozen
`X/Z` plus nearest-neighbor `ZZ` generator set does not close to that basis.

The future audit is limited to 36 label-blind development identities across
G01-G12, F0/F1/F2, candidate 2 of decision set 1; it excludes every existing
D006-D049 feature row, calibration, and final-test data. It fixes seven proper
rotations, `eps_a=1e-12 km/s^2`, and `y_scale=20 m/s` without changing the
...
campaign result. The stop record is
`data/processed/reporting/post_gate5_d041_hefrk/technical_stop_timeout.json`.
It is not scientific evidence: partial output is unusable, no calibration or
final-test rows were read, and no hardware/GPU, mission-loop, or Gate 6 work
occurred. D041-C1 accepts one execution-only correction that caches shared RBF
distance and landmark computations; the scientific q/eta grid, channels,
residuals, controls, thresholds, folds, seeds, and data scope are unchanged.

D041 is a valid scientific negative. HEFRK-08-E050 scored mean pooled OOF
NRMSE `0.00659197` versus C06 `0.00683281`, with paired difference
`-0.000240838` and 95% interval `[-0.000241783,-0.000239875]`. The paired
interval is below zero, but the improvement was about 3.52%, below the frozen
5% rule; the best declared ablation, HEFRK-08-E075 at `0.00652507`, was still
about 4.50% below C06. The matched TWO-RBF-08-E050 control scored `0.00672438`,
so no 5%-over-classical-control condition passed. D041 therefore remains a
valid development-only negative, not an invented-QML success. All 39,000
development rows, five outer folds, four inner folds, 20 seeds, 900 channel
audits, and 900 projection audits completed with zero validation-outcome use,
zero group overlap, zero calibration/final-test reads, and zero hardware/GPU,
mission-loop, and Gate 6 activity. RFIG-068 and RFIG-069 are the paper-ready
figures. The useful signal is that fixed hybrid fusion reduced error relative
to C06, but not enough to establish a thresholded or quantum-specific gain.

D042/P010 is the next prospective invention protocol. It tests an Adaptive
Gated Error-Conditioned Fidelity-RBF Kernel (AGEFRK). The gate is fixed before
campaign fitting and uses only the unchanged outer-fold C06 feasibility
probability: `eta(x) = 0.25 + 0.50 * (1-p_C06(x))`. The AGEFRK correction mixes
the D041 fidelity and RBF-0.25 channels with this row-wise eta. The matched
